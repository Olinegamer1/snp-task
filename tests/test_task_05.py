...
class TestDateInFuture(unittest.TestCase):
    ...
    def test_five_days_in_future(self):
        self.assertEqual(date_in_future(5),
                         (datetime.now() + timedelta(days=5)).strftime(FORMAT_DATE))

    def test_today(self):
        self.assertEqual(date_in_future(0), datetime.now().strftime(FORMAT_DATE))

    def test_yesterday(self):
        self.assertEqual(date_in_future(-1),
                         (datetime.now() + timedelta(days=-1)).strftime(FORMAT_DATE))
